                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between bg-gray-50 p-3 rounded-lg border border-gray-100">
                                        <div className="flex items-center gap-2">
                                            <input
                                                type="checkbox"
                                                id={`cashflow-toggle-${index}`}
                                                checked={loan.includeInCashflow !== false}
                                                onChange={(e) => onUpdate(index, 'includeInCashflow', e.target.checked)}
                                                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                                            />
                                            <label htmlFor={`cashflow-toggle-${index}`} className="text-sm text-gray-700 cursor-pointer">
                                                Im Cashflow berücksichtigen
                                            </label>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-sm font-bold text-gray-900">{formatCurrency(result.rate)} / Monat</div>
                                            <div className="text-[10px] text-gray-500">Volumen: {formatCurrency(result.betrag)}</div>
                                        </div>
                                    </div>
                                </div>
                            )}
